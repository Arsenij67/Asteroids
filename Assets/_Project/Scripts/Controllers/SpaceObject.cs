...
namespace Asteroid.SpaceObjectActions
{
    public class SpaceObject : MonoBehaviour
    {
        ...
        public bool TryTeleport(Vector2 position)
        {
            ...
            {
                newPos = new Vector2(UpRightBorder.x, position.y);
            }
            else if (position.x > UpRightBorder.x)
            {
                newPos = new Vector2(DownLeftBorder.x, position.y);
            }
            else if (position.y < DownLeftBorder.y)
            {
                newPos = new Vector2(position.x, UpRightBorder.y);
            }
            else if (position.y > UpRightBorder.y)
            {
                newPos = new Vector2(position.x, DownLeftBorder.y);
            }
